class RevalidationPdf
  ...
  def customs_broker_name
    @bl_house_line.customs_broker&.name || customs_agent_name
  end
  ...
  def almacen_name
    @bl_house_line.container&.almacen.to_s.strip
  end

  def terminal_name
    @bl_house_line.container&.recinto.to_s.strip
  end

...
RSpec.describe "BlHouseLines", type: :request do
  ...
  describe "GET /bl_house_lines" do
    ...
    it "excludes old records by default date range (last 60 days)" do
      sign_in user, scope: :user
      ...
      old_line.update_column(:created_at, 75.days.ago)
  ...
  describe "PATCH /bl_house_lines/:id" do
    context "with valid parameters" do
      ...
      it "recalculates BL-RECASU amount from formula on show flow update" do
        ...
        expect(response).to redirect_to(bl_house_line_url(bl_house_line, anchor: "servicios"))
        expect(service.reload.amount).to eq(2646)
        expect(service.observaciones).to eq("Editado RECASU")
      end
    end

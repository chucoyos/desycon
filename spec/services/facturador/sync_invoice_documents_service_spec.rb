...
RSpec.describe Facturador::SyncInvoiceDocumentsService, type: :service do
  describe '.call' do
    ...
    it 'stores xml and pdf attachments and creates events' do
      allow(client_double).to receive(:descargar_xml).and_return('<cfdi:Comprobante xmlns:cfdi="http://www.sat.gob.mx/cfd/4" Serie="GMZO" Folio="1" Fecha="2026-04-07T10:11:12" Moneda="MXN" TipoDeComprobante="I" FormaPago="99" MetodoPago="PPD"><cfdi:Emisor Rfc="PTM0701119T6"/></cfdi:Comprobante>')
      ...
      expect(invoice.payload_snapshot['tipoDeComprobante']).to eq('I')
      expect(invoice.payload_snapshot['moneda']).to eq('MXN')
